...
import {
  ...
  InvoiceStatus,
  InvoiceLineItem,
  CommissionSourceAnalysis,
  MarketPriceCommissionAdvice,
} from "../../types";
import {
...
export const AdminBillingPage: React.FC = () => {
  ...
  const [commissionAnalysisResult, setCommissionAnalysisResult] =
    useState<CommissionSourceAnalysis | null>(null);
  const [commissionProductQuery, setCommissionProductQuery] = useState("");
  const [commissionAdvice, setCommissionAdvice] =
    useState<MarketPriceCommissionAdvice | null>(null);

  const loadData = () => {
    setIsLoading(true);
    try {
      setShipments(
        JSON.parse(localStorage.getItem(SHIPMENTS_STORAGE_KEY) || "[]"),
      );
      setCompanies(
        JSON.parse(localStorage.getItem(MOCK_COMPANIES_STORAGE_KEY) || "[]"),
      );
      setInvoices(
        JSON.parse(localStorage.getItem(INVOICES_STORAGE_KEY) || "[]"),
      );
      setMatches(
        JSON.parse(localStorage.getItem(CONFIRMED_MATCHES_STORAGE_KEY) || "[]"),
      );
    } catch (e) {
  ...
  const billableShipments = useMemo(
    () =>
      shipments.filter(
        (s) =>
          s.status === ShipmentStatus.DELIVERED &&
          s.matches.some((m) => !m.billed),
      ),
    [shipments],
  );
    ...
    const companyMatchIds = new Set(
      shipments
        ...
        .filter(
          (m) =>
            !m.billed &&
            (m.demandDetails.submittedByCompanyId === selectedCompanyId ||
              m.stockDetails.uploadedByCompanyId === selectedCompanyId),
        )
        .map((m) => m.id),
    );
  ...
  const handleGenerateInvoice = async () => {
    if (!ai || !selectedCompanyId || unbilledMatchesForCompany.length === 0)
      return;
...
The response must be the plain text of the invoice, without any markdown formatting.`;
    ...
    try {
      ...
      console.error(e);
      setGeneratorError(
        t("adminBilling_error_invoiceGenerationFailed" as TranslationKey),
      );
    } finally {
      setIsAiLoading({});
    }
  ...
  const handleConfirmAndSaveInvoice = () => {
    ...
    const updatedMatches = matches.map((m) =>
      generatedInvoice.relatedMatchIds.includes(m.id)
        ? { ...m, billed: true, invoiceId: generatedInvoice.id }
        : m,
    );
    ...
    alert(
      t("adminBilling_invoiceSaveSuccess" as TranslationKey, {
  ...
  const handleAnalyzeCommissions = async () => {
    ...
    if (billedMatches.length === 0) {
      setCommissionAnalysisResult({
        summary: "No billed matches to analyze yet.",
        topProducts: [],
        topCustomers: [],
      });
      setIsAiLoading({});
  ...
  const handleGetCommissionAdvice = async () => {
    ...
    try {
      ...
      setIsAiLoading((prev) => ({ ...prev, advice: false }));
    }
  };
  ...
  const renderBillableTab = () => (
    <Card title={t("adminBilling_billable_title" as TranslationKey)}>
      <p className="text-sm text-slate-300 mb-4">
        {t("adminBilling_billable_desc" as TranslationKey)}
      </p>
      {billableShipments.length === 0 ? (
        <p className="text-slate-400">
          {t("adminBilling_noBillableShipments" as TranslationKey)}
        </p>
      ) : (
        <div className="space-y-3 max-h-[60vh] overflow-y-auto custom-scrollbar pr-2">
          {billableShipments.map((s) => {
            const unbilledInShipment = s.matches.filter((m) => !m.billed);
            const totalUnbilled = unbilledInShipment.reduce(
              (sum, m) => sum + m.commissionAmount,
              0,
            );
            const flatCompanies = unbilledInShipment
              .flatMap((m) => [
                {
                  id: m.demandDetails.submittedByCompanyId,
                  name: m.demandDetails.submittedByCompanyName,
                },
                {
                  id: m.stockDetails.uploadedByCompanyId,
                  name: m.stockDetails.uploadedByCompanyName,
                },
              ])
              .filter((c: any) => c && c.id && c.name) as {
              id: string;
              name: string;
            }[];

            const companiesInvolved = flatCompanies.reduce(
              (acc: { id: string; name: string }[], current) => {
            ...
            return (
              <div key={s.id} className="p-3 bg-slate-700/50 rounded-lg">
                <p className="font-semibold text-cyan-300">
                  {t("adminBilling_shipmentId" as TranslationKey)}:{" "}
                  {s.id.slice(-8)}
                </p>
                <p className="text-sm">
                  {t("adminBilling_unbilledCommission" as TranslationKey)}:{" "}
                  <span className="font-bold text-green-400">
                    {totalUnbilled.toFixed(2)} EUR
                  </span>
                </p>
                <div className="mt-2 text-xs">
                  <p className="font-medium text-slate-300">
                    {t("adminBilling_involvedCompanies" as TranslationKey)}:
                  </p>
                  <div className="flex flex-wrap gap-2 mt-1">
                    {companiesInvolved.map(
                      (c) =>
                        c.id && (
                          <Button
                            key={c.id}
                          ...
                          >
                            {t(
                              "adminBilling_generateInvoiceForCompany" as TranslationKey,
  ...
  const renderGeneratorTab = () => (
    <Card title={t("adminBilling_invoiceGenerator_title" as TranslationKey)}>
      <p className="text-sm text-slate-300 mb-4">
        {t("adminBilling_invoiceGenerator_desc" as TranslationKey)}
      </p>
      <Select
      ...
      {selectedCompanyId && (
        <div className="mt-4">
          <h4 className="font-semibold text-cyan-300 mb-2">
            {t("adminBilling_unbilledItemsForCompany" as TranslationKey, {
              companyName:
                companies.find((c) => c.id === selectedCompanyId)
                  ?.companyName || "",
            })}
          </h4>
          {unbilledMatchesForCompany.length === 0 ? (
            <p className="text-sm text-slate-400">
              {t("adminBilling_noUnbilledItemsForCompany" as TranslationKey)}
            </p>
          ) : (
            <div className="text-xs space-y-1 mb-3">
              {unbilledMatchesForCompany.map((m) => (
                <p key={m.id}>
                  - Match {m.id.slice(-6)}: {m.commissionAmount.toFixed(2)} EUR
                </p>
              ))}
            </div>
          )}
      ...
      {isAiLoading.invoice && (
        <LoadingSpinner
          text={t("adminBilling_generatingInvoice" as TranslationKey)}
        />
      )}
      {generatorError && (
        <p className="text-red-400 mt-2 text-sm">{generatorError}</p>
      )}
      {generatedInvoice && (
        <div className="mt-4 space-y-3">
          <h4 className="font-semibold text-cyan-300">
            {t("adminBilling_generatedDraft" as TranslationKey)}
          </h4>
          <Textarea
            value={generatedInvoice.invoiceText}
            readOnly
            rows={12}
            className="text-xs"
          />
          ...
          >
            {t("adminBilling_confirmAndSaveInvoice" as TranslationKey)}
          </Button>
        </div>
      )}
    </Card>
  ...
  const renderIssuedTab = () => (
    <Card title={t("adminBilling_issuedInvoices_title" as TranslationKey)}>
      {invoices.length === 0 ? (
        <p className="text-slate-400">
          {t("adminBilling_noInvoicesIssued" as TranslationKey)}
        </p>
      ) : (
        <div className="overflow-x-auto custom-scrollbar">
          <table className="w-full text-sm text-left">
            <thead className="text-xs text-slate-300 uppercase bg-slate-700/50">
              <tr>
                <th className="p-3">
                  {t("invoiceTable_id" as TranslationKey)}
                </th>
                <th className="p-3">
                  {t("invoiceTable_company" as TranslationKey)}
                </th>
                <th className="p-3">
                  {t("invoiceTable_issueDate" as TranslationKey)}
                </th>
                <th className="p-3">
                  {t("invoiceTable_dueDate" as TranslationKey)}
                </th>
                <th className="p-3 text-right">
                  {t("invoiceTable_amount" as TranslationKey)}
                </th>
                <th className="p-3 text-center">
                  {t("invoiceTable_status" as TranslationKey)}
                </th>
                <th className="p-3">
                  {t("invoiceTable_actions" as TranslationKey)}
                </th>
              </tr>
            </thead>
            <tbody>
                ...
                .map((inv) => (
                  <tr key={inv.id} className="border-b border-slate-700">
                    ...
                    <td className="p-3 text-right font-semibold">
                      {inv.totalAmount.toFixed(2)} EUR
                    </td>
                    <td className="p-3 text-center">
                      <span
                        className={`px-2 py-1 text-xs font-medium rounded-full text-white ${getStatusColor(inv.status)}`}
                      >
                    ...
                    <td className="p-3 flex gap-1">
                      <Button
                        ...
                        title={t("invoiceActions_view" as TranslationKey)}
                      >
                      ...
                      {inv.status === InvoiceStatus.DRAFT && (
                        ...
                        >
                          {t("invoiceActions_markSent" as TranslationKey)}
                        </Button>
                      )}
                      {inv.status === InvoiceStatus.SENT && (
                        ...
                        >
                          {t("invoiceActions_markPaid" as TranslationKey)}
                        </Button>
                      )}
                    </td>
                  </tr>
  ...
  const renderCommissionAnalysisTab = () => (
    <div className="space-y-6">
      <Card
        title={t("adminBilling_commissionAnalysis_title" as TranslationKey)}
      >
        <p className="text-sm text-slate-300 mb-4">
          {t("adminBilling_commissionAnalysis_desc" as TranslationKey)}
        </p>
        <AiFeatureButton
        ...
        {isAiLoading.analysis && (
          <LoadingSpinner
            text={t("adminBilling_analyzingCommissions" as TranslationKey)}
          />
        )}
        {commissionAnalysisResult && (
          <div className="mt-4 space-y-3 text-sm">
            <h4 className="font-semibold text-cyan-300">
              {t("adminBilling_commissionAnalysisResult" as TranslationKey)}
            </h4>
      ...
      <Card title={t("adminBilling_commissionAdvice_title" as TranslationKey)}>
        <p className="text-sm text-slate-300 mb-4">
          {t("adminBilling_commissionAdvice_desc" as TranslationKey)}
        </p>
        <Input
          ...
          placeholder={t(
            "adminBilling_productType_placeholder" as TranslationKey,
          )}
        />
        <AiFeatureButton
        ...
        {isAiLoading.advice && (
          <LoadingSpinner
            text={t("adminBilling_gettingAdvice" as TranslationKey)}
          />
        )}
        {commissionAdvice && (
          <div className="mt-4 space-y-2 text-sm p-3 bg-slate-700/50 rounded-lg">
            <h4 className="font-semibold text-cyan-300">
              {t("adminBilling_commissionAdviceResult" as TranslationKey)}
            </h4>
            <p>
              <strong>
                {t("adminBilling_productType_label" as TranslationKey)}:
              </strong>{" "}
              {commissionAdvice.productType}
            </p>
            <p>
              <strong>{commissionAdvice.suggestedCommissionRate}</strong> -{" "}
              {commissionAdvice.justification}
            </p>
            <p className="text-xs text-slate-300 italic mt-1">
              {commissionAdvice.marketPriceInsights}
            </p>
          </div>
        )}
      </Card>
  ...
  return (
    <>
      <PageTitle
        title={t("adminBilling_title" as TranslationKey)}
        subtitle={t("adminBilling_subtitle" as TranslationKey)}
        icon={<CurrencyDollarIcon className="h-8 w-8" />}
      />

      <div className="mb-6 flex space-x-2 border-b border-slate-700">
        <Button
          variant={activeTab === "billable" ? "primary" : "ghost"}
          onClick={() => setActiveTab("billable")}
          className="rounded-b-none"
        >
          {t("adminBilling_tab_billableShipments" as TranslationKey)}
        </Button>
        <Button
          ...
          className="rounded-b-none"
        >
          {t("adminBilling_tab_invoiceGenerator" as TranslationKey)}
        </Button>
        <Button
          ...
          className="rounded-b-none"
        >
          {t("adminBilling_tab_issuedInvoices" as TranslationKey)}
        </Button>
        <Button
          variant={activeTab === "commissionAnalysis" ? "primary" : "ghost"}
          onClick={() => setActiveTab("commissionAnalysis")}
          className="rounded-b-none"
        >
          {t("adminBilling_tab_commissionAnalysis" as TranslationKey)}
        </Button>
      </div>
      ...
      {viewingInvoice && (
        ...
        >
          <Card
            title={t("invoice_modal_title" as TranslationKey, {
