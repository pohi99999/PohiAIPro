...
export const AdminMatchmakingPage: React.FC = () => {
  ...
  const [suggestions, setSuggestions] = useState<
    MatchmakingSuggestion[] | string | null
  >(null);
  ...
  const activeDemands = allDemands.filter(
    (d) => d.status === DemandStatus.RECEIVED,
  );
  const availableStock = allStock.filter(
    (s) => s.status === StockStatus.AVAILABLE,
  );
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      const demandsData = demandsSnapshot.docs.map(
        (doc) => ({ id: doc.id, ...doc.data() }) as DemandItem,
      );
      const stockData = stockSnapshot.docs.map(
  ...
  const generateAutomaticPairingSuggestions = useCallback(async (): Promise<
    MatchmakingSuggestion[] | string
  > => {
    ...
    const matchmakingSchema = {
      ...
      items: {
        ...
        properties: {
          ...
          matchStrength: { type: Type.STRING },
          similarityScore: { type: Type.NUMBER },
        },
        required: ["demandId", "stockId", "reason"],
      },
    };
    ...
    try {
      ...
      return Array.isArray(parsed)
        ? parsed.map((item, i) => ({ ...item, id: `sugg-${i}-${Date.now()}` }))
        : t("customerNewDemand_error_aiResponseNotArray");
    } catch (error) {
      console.error(error);
  ...
  const rankStockForDemandWithGemini = useCallback(
    async (demand: DemandItem): Promise<RankedStock[] | string> => {
      if (!ai) return t("customerNewDemand_error_aiUnavailable");
      if (availableStock.length === 0)
        return t("adminMatchmaking_workbench_noRankedStock");
    ...
    try {
      ...
      if (isManual) {
        ...
        setManualMatchSuccess(
          t("adminMatchmaking_workbench_matchCreatedSuccess", {
            matchId: newMatch.id,
          }),
        );
  ...
  return (
    <>
      <PageTitle
        title={t("adminMatchmaking_title")}
        subtitle={t("adminMatchmaking_subtitle")}
      ...
      <div className="mb-6 flex space-x-1 border-b border-slate-700">
        ...
        >
          {t("adminMatchmaking_tab_aiSuggestions")}
        </Button>
        <Button
          ...
          className="rounded-b-none"
        >
          {t("adminMatchmaking_tab_manualWorkbench")}
        </Button>
      </div>
      ...
      {activeTab === "ai" && (
        <Card title={t("adminMatchmaking_requestMatchmakingSuggestions")}>
          ...
          >
            {t("adminMatchmaking_requestMatchmakingSuggestions")}
          </Button>
          ...
          {suggestions && !isSuggestionsLoading && (
            <div className="mt-4">
              <MatchmakingVisualization
                ...
                stockItems={allStock}
                onConfirmMatch={(sugg) =>
                  handleMatch(sugg.demandId, sugg.stockId, false)
                }
              />
            </div>
          )}
      ...
      {activeTab === "manual" && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-2 max-h-[70vh] overflow-y-auto custom-scrollbar pr-2">
              {activeDemands.map((demand) => (
                <div
                  ...
                  className={`p-3 rounded-md cursor-pointer border ${selectedDemand?.id === demand.id ? "bg-cyan-800/70 border-cyan-500" : "bg-slate-700/50 border-slate-600 hover:bg-slate-700"}`}
                >
                  <p className="font-semibold text-white">
                    {demand.productName || "N/A"}
                  </p>
                  ...
                  <p className="text-xs text-slate-400">
                    Ø{demand.diameterFrom}-{demand.diameterTo}cm,{" "}
                    {demand.length}m, {demand.quantity}pcs
                  </p>
                </div>
              ))}
            </div>
          ...
          <Card title={t("adminMatchmaking_workbench_stockTitle")}>
            {isRankingLoading && (
              <LoadingSpinner
                text={t("adminMatchmaking_workbench_loadingRankedStock")}
              />
            )}
            {!isRankingLoading && !selectedDemand && (
              <p className="text-slate-400 text-center py-8">
                {t("adminMatchmaking_workbench_selectDemandPrompt")}
              </p>
            )}
            {!isRankingLoading &&
              selectedDemand &&
              rankedStock.length === 0 && (
                <p className="text-slate-400 text-center py-8">
                  {t("adminMatchmaking_workbench_noRankedStock")}
                </p>
              )}
                  ...
                  >
                    <div className="flex justify-between items-start">
                      <div>
                        ...
                        <p className="text-sm text-slate-300">
                          <BuildingStorefrontIcon className="h-4 w-4 inline-block mr-1" />{" "}
                          {stock.uploadedByCompanyName}
                        </p>
                        <p className="text-xs text-slate-400">
                          Ø{stock.diameterFrom}-{stock.diameterTo}cm,{" "}
                          {stock.length}m, {stock.quantity}pcs
                        </p>
                        <p className="text-xs mt-1 italic text-yellow-300">
                          Reason: {stock.reason} (Score:{" "}
                      ...
                      <Button
                        size="sm"
                        onClick={() =>
                          handleMatch(selectedDemand!.id, stock.id!, true)
                        }
                      >
                        {t("adminMatchmaking_workbench_manualMatchButton")}
                      </Button>
                    </div>
                  </div>
                ))}
