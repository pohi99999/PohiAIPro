...
export const AdminTruckPlanningPage: React.FC = () => {
  ...
  const [costEstimation, setCostEstimation] = useState<CostEstimation | null>(
    null,
  );
  const [freightTips, setFreightTips] = useState<OptimizationTip[] | null>(
    null,
  );
  const [aiRouteSummary, setAiRouteSummary] = useState<string | null>(null);
  const [isAwaitingPlanGeneration, setIsAwaitingPlanGeneration] =
    useState(false);
  ...
  const loadData = useCallback(() => {
    ...
    try {
      ...
      const allMatches: ConfirmedMatch[] = matchesRaw
        ? JSON.parse(matchesRaw)
        : [];
      setUnassignedMatches(
        allMatches
          .filter((m) => !m.shipmentId)
          .sort(
            (a, b) =>
  ...
  const totalStagedVolume = useMemo(
    () =>
      stagedMatches.reduce(
        (sum, m) => sum + (m.stockDetails.cubicMeters || 0),
        0,
      ),
    [stagedMatches],
  );
  ...
  const handleStageMatch = (matchId: string) => {
    ...
    if (
      match &&
      totalStagedVolume + (match.stockDetails.cubicMeters || 0) <=
        selectedTruck.capacityM3
    ) {
  ...
  const handleAutofillTruck = async () => {
    ...
    const availableMatchesForPrompt = unassignedMatches
      ...
      .map((m) => {
        const origin = companies.find(
          (c) => c.id === m.stockDetails.uploadedByCompanyId,
        );
        const destination = companies.find(
          (c) => c.id === m.demandDetails.submittedByCompanyId,
        );
        return {
          id: m.id,
...
The response MUST ONLY contain the JSON object.`;
      ...
      const response: GenerateContentResponse = await ai.models.generateContent(
        {
          ...
          config: {
            ...
            responseSchema: {
              type: Type.OBJECT,
              properties: {
                shipmentIds: { type: Type.ARRAY, items: { type: Type.STRING } },
              },
            },
          },
        },
  ...
  const handleGeneratePlan = useCallback(async () => {
    ...
    const waypointsData = stagedMatches
      .flatMap((m) => {
        const manufacturer = companies.find(
          (c) => c.id === m.stockDetails.uploadedByCompanyId,
        );
        ...
        return [
          {
            ...
            items: [
              {
                name: m.stockDetails.productName,
                stockId: m.stockDetails.id,
                volumeM3: m.stockDetails.cubicMeters,
              },
            ],
          },
          {
            ...
            items: [
              {
                name: m.demandDetails.productName,
                demandId: m.demandDetails.id,
                volumeM3: m.demandDetails.cubicMeters,
              },
            ],
          },
        ];
      ...
      .filter(
        (wp) =>
          wp.company &&
          wp.company.address?.latitude &&
          wp.company.address?.longitude,
      );
  ...
  const handleGenerateOptimalRoute = async () => {
    ...
    try {
      ...
      const result: { matchIdsToStage: string[]; routeSummary: string } =
  ...
  const handleConfirmAndDispatch = () => {
    ...
    const newShipment: Shipment = {
      ...
      truckDetails: {
        ...
        capacityM3: selectedTruck.capacityM3,
      },
      ...
      ).toISOString(), // Dummy ETA
      plan: generatedPlan,
    };

    const allShipments: Shipment[] = JSON.parse(
      localStorage.getItem(SHIPMENTS_STORAGE_KEY) || "[]",
    );
    localStorage.setItem(
      SHIPMENTS_STORAGE_KEY,
      JSON.stringify([newShipment, ...allShipments]),
    );

    const allMatches: ConfirmedMatch[] = JSON.parse(
      localStorage.getItem(CONFIRMED_MATCHES_STORAGE_KEY) || "[]",
    );
    const updatedMatches = allMatches.map((m) =>
      stagedMatchIds.has(m.id) ? { ...m, shipmentId } : m,
    );
    localStorage.setItem(
      CONFIRMED_MATCHES_STORAGE_KEY,
  ...
  return (
    <>
      <PageTitle
        title={t("commandCenter_title")}
        subtitle={t("commandCenter_subtitle")}
        icon={<ComputerDesktopIcon className="h-8 w-8" />}
      />
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="space-y-6">
          <Card
            title={t("commandCenter_unassignedShipments")}
            bodyClassName="p-2"
          >
              ...
              {isLoading ? (
                ...
                unassignedMatches
                  ...
                  .map((match) => (
                    <div
                      key={match.id}
                      className="p-2 bg-slate-700/50 rounded flex justify-between items-center text-xs"
                    >
                      <div>
                        <p className="font-bold">
                          {match.demandDetails.productName}
                        </p>
                        <p>
                          {match.stockDetails.uploadedByCompanyName} &rarr;{" "}
                          {match.demandDetails.submittedByCompanyName}
                        </p>
                        <p className="text-cyan-400">
                          {match.stockDetails.cubicMeters?.toFixed(2)} m³
                        </p>
                      </div>
                      <Button
                        size="sm"
                        onClick={() => handleStageMatch(match.id)}
                      >
                        {t("commandCenter_stageButton")}
                      </Button>
                    </div>
                  ))
              )}
          ...
          <Card title={t("commandCenter_truckStaging")}>
            <div className="flex justify-between items-center mb-3">
              <Select
                ...
                onChange={(e) =>
                  setSelectedTruck(
                    VIRTUAL_TRUCKS.find((t) => t.id === e.target.value) ||
                      VIRTUAL_TRUCKS[0],
                  )
                }
              ...
              <div className="text-right ml-4">
                <p className="text-xs text-slate-400">
                  {t("commandCenter_capacity")}
                </p>
                <p className="font-bold text-lg">
                  {selectedTruck.capacityM3} m³
                </p>
              </div>
            </div>
            ...
            <div className="mt-3 max-h-[200px] overflow-y-auto custom-scrollbar space-y-1 pr-2">
              {stagedMatches.length === 0 ? (
                <p className="text-sm text-slate-400 text-center">
                  {t("commandCenter_stagingEmpty")}
                </p>
              ) : (
                stagedMatches.map((match) => (
                  <div
                    key={match.id}
                    className="text-xs p-1.5 bg-slate-700 rounded flex justify-between items-center"
                  >
                    <p>
                      {match.id.slice(-6)}:{" "}
                      {match.stockDetails.cubicMeters?.toFixed(2)}m³
                    ...
                    <Button
                      size="sm"
                      variant="danger"
                      onClick={() => handleUnstageMatch(match.id)}
                      className="!py-0.5 !px-1.5"
                    >
                      <ArrowUturnLeftIcon className="h-3 w-3 mr-1" />{" "}
                      {t("commandCenter_unstageButton")}
                    </Button>
                  </div>
                ))
              )}
            ...
            <div className="mt-4 grid grid-cols-2 gap-2">
              <AiFeatureButton
                text={t("commandCenter_autofill")}
                onClick={handleAutofillTruck}
                isLoading={isAiLoading.autofill}
                size="sm"
            ...
            />
            {isAiLoading.optimalRoute && (
              <LoadingSpinner
                text={t(
                  "commandCenter_generatingOptimalRoute" as TranslationKey,
                )}
              />
            )}
          </Card>
        ...
        <div className="space-y-6">
          <Card title={t("commandCenter_generatedPlan")}>
            {aiRouteSummary && !generatedPlan && (
              <div className="p-3 bg-slate-700/50 rounded-md mb-4 text-sm italic flex items-start gap-2">
                <LightBulbIcon className="h-4 w-4 text-yellow-300 shrink-0 mt-0.5" />{" "}
                <span>
                  <strong>
                    {t("commandCenter_aiRouteSummary" as TranslationKey)}:
                  </strong>{" "}
                  {aiRouteSummary}
                </span>
              </div>
            ...
            {isAiLoading.plan && (
              <LoadingSpinner text={t("commandCenter_generatingPlan")} />
            )}

            {!generatedPlan && !isAiLoading.plan && (
              <div className="text-center py-8 text-slate-500">
                {t("commandCenter_planWillAppearHere")}
              </div>
            )}
              ...
              <div className="mt-4 space-y-4">
                <SimulatedRouteMap
                  ...
                  optimizedRouteDescription={
                    generatedPlan.optimizedRouteDescription
                  }
                />
                <Button
          ...
          <Card title={t("commandCenter_planAnalysis")}>
            <div className="space-y-4">
              ...
              <div>
                ...
                />
                {isAiLoading.tips && (
                  <LoadingSpinner text={t("commandCenter_gettingTips")} />
                )}
                {freightTips && (
                  <div className="mt-3 p-3 bg-slate-700/50 rounded-lg text-sm">
                    <h4 className="font-semibold text-cyan-300 mb-2">
                      {t("commandCenter_freightTipsResult")}
                    </h4>
                    <ul className="space-y-2">
                      {freightTips.map((tip) => (
                        ...
                        >
                          <LightBulbIcon className="h-4 w-4 text-yellow-400 shrink-0 mt-0.5" />
                          <span>{tip.tip}</span>
                        </li>
                      ))}
                    </ul>
