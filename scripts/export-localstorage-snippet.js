...
(function exportPohiLocalStorage(){
  ...
  if (!hasAny) {
    if (!confirm('No known Pohi localStorage keys found. Export ALL localStorage items instead?')) return;
    for (let i=0;i<localStorage.length;i++){ const k = localStorage.key(i); try { out[k]=JSON.parse(localStorage.getItem(k)); } catch(e){ out[k]=localStorage.getItem(k); } }
  }
